 ...
 *
 * <p>{@link wpiPoseEstimator#update} should be called every robot loop.
 *
 * <p>{@link wpiPoseEstimator#addVisionMeasurement} can be called as infrequently as you
...
public class wpiPoseEstimator {
    ...
    public void setVisionMeasurementStdDevs(Matrix<N3, N1> visionMeasurementStdDevs) {
        ...
        for (int row = 0; row < 3; ++row) {
            if (m_q.get(row, 0) == 0.0) {
                ...
                System.out.println("row: " + row + ", value: " + m_q.get(row, 0) / (m_q.get(row, 0) + Math.sqrt(m_q.get(row, 0) * r[row])));
                m_visionK.set(
    ...
    public void RanTest(Pose2d visionDistances, double timestampSeconds) {
        var sample = m_poseBuffer.getSample(timestampSeconds);
        if (sample.isEmpty()) {
            return;
        }

        Pose2d samplePose = sample.get().poseMeters;

        Pose2d calculated_pose = new Pose2d(
                samplePose.getX() - visionDistances.getX(),
                samplePose.getY() - visionDistances.getY(),
                Rotation2d.fromDegrees(samplePose.getRotation().getDegrees() + visionDistances.getRotation().getDegrees())
        );

        // System.out.println(String.format("c_p x: %f, c_p y: %f, c_p angle: %f", calculated_pose.getX(), calculated_pose.getY(), calculated_pose.getRotation().getDegrees()));

        this.targetPose = calculated_pose;
    }
     ...
     * wpiPoseEstimator#update} every loop.
     *
     ...
     *                              wpiPoseEstimator#updateWithTime(double, Rotation2d, SwerveModulePosition[])} then you
    ...
    public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
        SmartDashboard.putBoolean("not filtering vision", false);
    ...
    public void addVisionMeasurementWithoutFilter(Pose2d visionRobotPoseMeters, double timestampSeconds) {
        System.out.println("using vision without filtering");
        SmartDashboard.putBoolean("not filtering vision", true);
        ...
        m_odometry.resetPosition(
                sample.get().gyroAngle,
                sample.get().modulePositions,
                sample.get().poseMeters.exp(twist));
     ...
     * wpiPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
     *
     * @param visionRobotPoseMeters    The pose of the robot as measured by the vision camera.
     * @param timestampSeconds         The timestamp of the vision measurement in seconds. Note that if you
     *                                 don't use your own time source by calling {@link
     *                                 wpiPoseEstimator#updateWithTime(double, Rotation2d, SwerveModulePosition[])}, then
