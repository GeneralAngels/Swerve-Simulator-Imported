...
import frc.robot.subsystems.NewDrive.CheesySwerveModuleState;
 ...
 */
public class CheesySwerveDriveKinematics {
    private final SimpleMatrix m_inverseKinematics;
    private final SimpleMatrix m_forwardKinematics;

    private final int m_numModules;
    private final CheesyTranslation2d[] m_modules;
    private CheesyTranslation2d m_prevCoR = new CheesyTranslation2d();
    private final CheesyRotation2d[] m_rotations;
     ...
     */
    public CheesySwerveDriveKinematics(CheesyTranslation2d... wheelsMeters) {
        ...
        m_rotations = new CheesyRotation2d[m_numModules];
        ...
        for (int i = 0; i < m_numModules; i++) {
            ...
            m_rotations[i] = new CheesyRotation2d(m_modules[i].x(), m_modules[i].y(), true);
        }
     ...
     *     attainable max velocity. Use the {@link #desaturateWheelSpeeds(CheesySwerveModuleState[], double)
     *     DesaturateWheelSpeeds} function to rectify this issue.
     */
    @SuppressWarnings("LocalVariableName")
    public CheesySwerveModuleState[] toSwerveModuleStates(
            CheesyChassisSpeeds chassisSpeeds, CheesyTranslation2d centerOfRotationMeters) {
        ...
        CheesySwerveModuleState[] moduleStates = new CheesySwerveModuleState[m_numModules];
        ...
        for (int i = 0; i < m_numModules; i++) {
            ...
            CheesyRotation2d angle = new CheesyRotation2d(x, y, true);

            moduleStates[i] = new CheesySwerveModuleState(speed, angle);
        }
    ...
    /**
     * Performs inverse kinematics. See {@link #toSwerveModuleStates(CheesyChassisSpeeds, CheesyTranslation2d)}
     * toSwerveModuleStates for more information.
     *
     * @param chassisSpeeds The desired chassis speed.
     * @return An array containing the module states.
     */
    public CheesySwerveModuleState[] toSwerveModuleStates(CheesyChassisSpeeds chassisSpeeds) {
        return toSwerveModuleStates(chassisSpeeds, new CheesyTranslation2d());
    }
     ...
     */
    public CheesyChassisSpeeds toChassisSpeeds(CheesySwerveModuleState... wheelStates) {
        ...
        return new CheesyChassisSpeeds(
                chassisSpeedsVector.get(0, 0),
                chassisSpeedsVector.get(1, 0),
                chassisSpeedsVector.get(2, 0));
    }

    public CheesyChassisSpeeds toChassisSpeedWheelConstraints(CheesySwerveModuleState... wheelStates) {
        if (wheelStates.length != m_numModules) {
            throw new IllegalArgumentException(
                    "Number of modules is not consistent with number of wheel locations provided in "
                            + "constructor");
        ...
        for (int i = 0; i < m_numModules; i++) {
            ...
            var beta =
                    module.angle.rotateBy(
                            m_rotations[i].inverse()).rotateBy(CheesyRotation2d.fromRadians(Math.PI / 2.0));
    ...
    public static void desaturateWheelSpeeds(
            CheesySwerveModuleState[] moduleStates, double attainableMaxSpeedMetersPerSecond) {
        double realMaxSpeed = Collections.max(Arrays.asList(moduleStates)).speedMetersPerSecond;
        if (realMaxSpeed > attainableMaxSpeedMetersPerSecond) {
            for (CheesySwerveModuleState moduleState : moduleStates) {
    ...
    public final CheesyTranslation2d[] getModuleLocations() {
        return m_modules;
    }
    ...
    public static CheesyChassisSpeeds compensateForSkew(CheesyChassisSpeeds speeds) {
        if (CheesyUtil.epsilonEquals(speeds.omegaRadiansPerSecond,0, 1e-2)) return speeds;

        double kFactor = 0.1;
        double magnitude =
                Math.hypot(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond) * speeds.omegaRadiansPerSecond * kFactor;

        CheesyTranslation2d t = new CheesyTranslation2d(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond).rotateBy(
                CheesyRotation2d.fromDegrees(Math.signum(-speeds.omegaRadiansPerSecond) * 90.0));

        // t is now right direction, set right magnitude.
        t.scale(magnitude / t.norm());

        CheesyChassisSpeeds r = new CheesyChassisSpeeds(speeds.vxMetersPerSecond + t.x(),
